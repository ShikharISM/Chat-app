...
const ChatContainer = ({selectedUser, setselectedUser}) => {
    ...
    useEffect(() => {
      if(scrollEnd.current){
        scrollEnd.current.scrollIntoView({}) // to get the latest message view when clicked
      }
    },[])
    return  selectedUser ? (
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
                ...
                <img onClick={assets.help_icon} src={assets.arrow_icon} className='md:hidden max-w-7' alt="" />
            ...
            <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
                {
                    messagesDummyData.map((msg,index)=>(
                        <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'} `}>
                            ...
                            <img src={msg.senderId === '680f50e4f103cd28382ecf9' ? assets.avatar_icon : assets.profile_martin} className='w-7 rounded-full' alt="" />
